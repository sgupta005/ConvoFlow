...
/**
 * Find the workspace root node_modules (for monorepo support)
 */
const findWorkspaceRoot = () => {
  let currentDir = path.resolve(__dirname, "..")

  // Traverse up until we find a node_modules with .pnpm or reach root
  while (currentDir !== path.parse(currentDir).root) {
    ...
    // Also check for pnpm-workspace.yaml as a workspace root indicator
...
const findPackageFiles = () => {
  const nodeModules = findWorkspaceRoot()
